...
#define MSG_ID_UPD_SOS 0x14
	...
	/*!
	* ACK-ACK Message Structure
	* Message Acknowledged
	* ID: 0x05  0x01 Payload Length=2 bytes
	*/
	PACK(
	struct AckAck {
	...
	* ID: 0x05  0x00 Payload Length=2 bytes
	*/
	PACK(
	struct AckNak {
		UbloxHeader header;     //!< Ublox header
	...
	*/
	...
	struct CfgAnt {
		...
		uint32_t pins;          //!< Antenna Pin Configuration
		uint8_t checksum[2];    //!< Checksum
	});
	...
	*/
	...
	struct CfgDat {
		UbloxHeader header;  //!< Ublox header
		double maja;         //!< Semi-major Axis
		double flat;         //!< Flattening
		float dx;            //!< X Axis shift at the origin
		float dy;            //!< Y Axis shift at the origin
		float dz;            //!< Z Axis shift at the origin
		float rotx;          //!< Rotation about the X Axis
		float roty;          //!< Rotation about the Y Axis
		float rotz;          //!< Rotation about the Z Axis
		float scale;         //!< Scale change
		uint8_t checksum[2]; //!< Checksum
	});
	...
	*/
	...
	/*!
	* CFG-DYNSEED Message Structure
	* Program the dynamic seed for the host interface signature.
	* If successfully configured, the message will answer with ACK,
	* otherwise with NAK. Before 	the first programming, it is assumed
	* that the dynamic seed is all '0'..
	* ID: 0x06  0x085 Payload Length=12 bytes
    */
	PACK(
	struct CfgDynseed{
		UbloxHeader header;
		uint8_t version;        // Message version (0 for this version)
		uint8_t reserved1;      // Reserved
		uint32_t seedhi;	    //high word of dynamic seed
		uint32_t seedlo;	    //low word of dynamic seed
		uint8_t checksum[2];    //**$**Should this be included?
	});
	...
	*/
	...
	/*!
	* CFG-FIXSEED Message Structure
	* Program the fixed seed for the host interface signature.
	* Moreover it will configure the set of messages that 
	* will be signed (min. 1, max. 10). If the 	class ID of the message 
	* is 0 the configuration is ignored for that message. If 
	* successfully configured, the message will answer with ACK, 
	* otherwise with NAK. Header Class ID Length (Bytes) Payload.
	* ID: 0x06  0x084 Payload Length=4+36*numSources bytes
	*/
	PACK(
	struct CfgFixseedRepeated{
		uint8_t classid;         // Class ID on the message
		uint8_t msgId;           // Message ID on the message
	});
	PACK(
	struct CfgFixseed{
		UbloxHeader header;
		uint8_t version;        // Message version (0 for this version)
		uint8_t length;         // Number of sources (affects length of this message)
		uint8_t reserved1;        // Reserved
		CfgFixseedRepeated repeated_block[MAX_NUM_SOURCES];
		uint8_t checksum[2];    //**$**Should this be included?
	});

	/*!
	* CFG-GEOFENCE Message Structure
	...
	* ID: 0x06  0x069 Payload Length=8 + 12*numFences bytes
	*/
	PACK(
	struct CfgGeofenceRepeated{
		int32_t lat;         // Latitude of the geofence circle center
		int32_t lon;         // Longitude of the geofence circle center
		uint32_t radius;     // Radius of the geofence circle
	});
	PACK(
	struct CfgGeofence{
		UbloxHeader header;
		uint8_t version;        // Message version (0 for this version)
		uint8_t numFences;      // Number of geofences contained in this message.
		uint8_t confLvl;        // Required confidence level for state evaluation.
		uint8_t reserved1;      // Reserved
		...
		uint8_t reserved2;      // Reserved
		CfgGeofenceRepeated repeated_block[MAX_NUM_SOURCES];
		uint8_t checksum[2];    //**$**Should this be included?
	});
	...
	*/
	...
	/*!
	* CFG-HNR Message Structure
	* The u-blox receivers support high rates of navigation update up to 20 Hz. The navigation
	...
	* ID: 0x06  0x05C Payload Length=4 bytes
	*/
	PACK(
	struct CfgHnr{
		UbloxHeader header;
		uint8_t highNavRate;    // Rate of navigation solution output
	...
	* ID: 0x06  0x02 Payload Length=1 bytes
	*/
	PACK(
	struct CfgInf{
		UbloxHeader header;
		uint8_t protocolID;      // Protocol Identifier, identifying the output
								 // protocol for this Poll Request.The following are valid Protocol Identifiers :
								 //	    0 : UBX Protocol
								 //     1 : NMEA Protocol
								 //     2 - 255 : Reserved
		uint8_t checksum[2];    //**$**Should this be included?
	...
	*/
	...
	struct CfgPm2V0{
		UbloxHeader header;
		uint8_t version;                   // Message version (0x1 for this version)
		uint8_t reserved1;                 // Reserved
		uint8_t maxStartupStateDur;        // Maximum time to spend in Acquisition state.
		uint8_t reserved2;                 // Reserved
		uint32_t flags;                    // PSM configuration flags
		uint32_t updatePeriod;             // Position update period. If set to 0, the receiver will never retry a fix and it will wait for external events
		uint32_t searchPeriod;             // Acquisition retry period if previously failed. If set to 0, the receiver will never retry a startup
		uint32_t gridOffset;               // Grid offset relative to GPS start of week
		uint16_t onTime;                   // Time to stay in Tracking state
		uint16_t minAcqTime;               // minimal search time
		uint8_t reserved3[20];             // Reserved
		uint8_t checksum[2];
	});
	/*!
	* CFG-PM2 Message Structure
	* NEWEST MESSAGE VERSION Extended Power Management configuration
	* ID: 0x06  0x3B Payload Length=48 bytes
	*/
	...
	struct CfgPm2V1{
		...
		uint16_t minAcqTime;               // minimal search time
		uint8_t reserved3[20];             // Reserved
		uint32_t extintInactivityMs;       // inactivity time out on EXTINT pint if enabled
		uint8_t checksum[2];
	});
	...
	*/
	...
	struct CfgPms{
		UbloxHeader header;
		uint8_t version;        // Message version (0 for this version)
		uint8_t powerSetupValue;// Power setup value
		uint16_t period;        // Position update period and search period.
	...
	*/
	...
	struct CfgPrtUART {
		UbloxHeader header;     // Ublox header
		uint8_t port_id;        // port identifier (0 or 1 for USART )
	...
	*/
	...
	struct CfgPrtSPI {
		...
		uint8_t reserved;       // reserved
		uint16_t tx_ready;      // transmit ready status
        uint32_t mode;          // SPI Mode Flagss
		uint8_t reserved2[4];   // reserved
		uint16_t inProtoMask;   // input protocol mask
		uint16_t outProtoMask;  // output protocol mask
		uint16_t flags;         // flags bit masks
		uint8_t reserved3[2];   // reserved
		uint8_t checksum[2];
	});
	...
	*/
	...
	struct CfgPwr{
		...
		uint8_t reserved1[3];        // Reserved
		uint32_t state;        // Enter system state. 
	...
	*/
	...
	struct CfgSbas {
		...
		uint8_t maxSBAS;        //!< Maximum Number of SBAS prioritized tracking channels (valid range: 0 - 3) to use (obsolete and superseeded by UBX-CFG-GNSS in protocol versions 14+).
        uint8_t scanmode2;      //!< Continuation of scanmode bitmask
        uint32_t scanmode1;     //!< Which SBAS PRN numbers to search for (Bitmask) If all Bits are set to zero, auto-scan (i.e. all valid PRNs) are searched. Every bit corresponds to a PRN number
		uint8_t checksum[2];
	});
...
#define NAV_SVINFO_FLAGS_PR_SMOOTHED 0B10000000 // Carrier Smoothed pseudorange used (PPP)
	...
	*/
	...
	enum Message_ID
	{
		ACK_ACK = 0x0501,               // (ID 0x05 0x1) Acknowledged
		ACK_NAK = 0x0500,               // (ID 0x05 0x1) Not-Acknowledged

		/*All UBX-AID messages are deprecated; use UBX-MGA messages instead*/
		AID_ALM = 0x0B30,               // (ID 0x0B 0x30) Almanac
		AID_AOP = 0x0B33,               // (ID 0x0B 0x33) Autonomous
		AID_EPH = 0x0B31,               // (ID 0x0B 0x31) Ephemerides              
		AID_HUI = 0x0B02,               // (ID 0x0B 0x02) GPS Health, Ionospheric, UTC
		AID_INI = 0x0B01,               // (ID 0x0B 0x01) Position, Time, Frequency, Clock Drift
